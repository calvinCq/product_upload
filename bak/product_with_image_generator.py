...
from utils.exceptions import ValidationError, OperationError, catch_exceptions
...
class ProductWithImageGenerator:
    ...
    def __init__(self, wechat_api_client=None, volcano_config=None, config_path: str = None):
        ...
        if not self.wechat_api and WECHAT_API_AVAILABLE:
            ...
            if api_config.get("appid") and api_config.get("appsecret"):
                ...
                self.logger.warning("缺少微信小店API配置")
    ...
    def generate_images_and_upload_product(self, product_description: str, 
        ...
        try:
            ...
            self.logger.info(f"成功生成 {len(main_image_paths)} 张主图和 {len(detail_image_paths)} 张详情图")
            
            # 2. 上传图片到微信小店
            self.logger.info("开始上传图片到微信小店...")
            ...
            self.logger.info(f"成功上传 {len(main_image_urls)} 张主图和 {len(detail_image_urls)} 张详情图")
            ...
            # 4. 上传商品到微信小店
            self.logger.info(f"开始上传商品到微信小店... (商品类型: {'视频号小店' if is_video_shop else '传统小店'})")
            ...
            self.logger.info(f"商品上传完成: {result.get('product_id')}")
            return result
        ...
        for image_path in image_paths:
            try:
                ...
                if upload_result.get("errcode") == 0 and "image_url" in upload_result:
                    ...
                    self.logger.info(f"图片上传成功: {image_path} -> {upload_result['image_url']}")
                else:
                    logger.error(f"图片上传失败: {image_path}, 错误: {upload_result}")
                    
            except Exception as e:
                self.logger.error(f"上传图片 {image_path} 时发生错误: {str(e)}")
    ...
    def _update_product_data_with_images(self, product_data: Dict[str, Any], 
        ...
        self.logger.info("商品数据已更新，添加了图片URL")
        return updated_data
    ...
    def _upload_product(self, product_data: Dict[str, Any], is_video_shop: bool = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"上传商品时发生错误: {str(e)}")
            raise
    ...
    def generate_images_only(self, product_description: str) -> Dict[str, List[str]]:
        ...
        Raises:
            ...
            OperationError: 当生成图片失败时
        """
        # 参数验证
        if not isinstance(product_description, str):
            self.logger.error("商品描述类型无效，必须是字符串格式")
            raise ValidationError("商品描述必须是字符串格式")
        
        if not product_description.strip():
            self.logger.error("商品描述不能为空")
            raise ValidationError("商品描述不能为空")
        
        try:
            # 确保图片保存目录存在
            os.makedirs(self.image_save_dir, exist_ok=True)
            
            self.logger.info("开始生成商品主图...")
            main_image_paths = self.image_generator.generate_product_images(
                product_description=product_description,
                count=self.main_images_count,
                image_type="main",
