...
def load_content_with_llm(file_path: str, config_file=None) -> dict:
    ...
    # 初始化钱多多API文本生成器（强制使用）
    try:
        text_generator = VolcanoTextGenerator(config_file=config_file)
...
def main():
    ...
    try:
        ...
        print("钱多多API配置检查完成")
        print(f"使用模型: {qianduoduo_config.get('model_name')}")
    except Exception as e:
    ...
    # 创建整合器，使用配置管理器（已通过配置管理器设置使用钱多多API）
    integrator = ImageGenerationIntegrator(
